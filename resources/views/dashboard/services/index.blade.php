...
@section('content')
    ...
    <div class="box">
        <div class="box-header">
            ...
            <a  href="{{ route('services.create') }}" style="display:inline-block; margin-left:1%" class="btn btn-primary btn-sm pull-right" >
        ...
        <div class="box-body">
            <table id="services-table" class="table table-bordered table-hover text-center">
                <thead>
                    <tr>
                ...
                <tbody>
                    @foreach ($services as $service)
                        <tr>
                            ...
                            <td>{{ $service->name }}</td>
                            <td>{{ str_limit($service->description, 40) }}</td>
                            <td>
                                @permission('services-read')
                                    <a class="btn btn-info btn-xs" href="{{ route('services.show', $service->id) }}"><i class="fa fa-eye"></i> @lang('global.show') </a>
                                @endpermission
                                ...
                                @permission('services-delete')
                                    <a href="{{ route('services.show', $service->id) }}?delete=true" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> @lang('global.delete') </a>
                                @endpermission
                            </td>
                        </tr>
                    @endforeach
