...
@section('content')
    @component('partials._breadcrumb')
        @slot('title', ['global.team', 'global.add'])
            ...
            <div class="col-md-12">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title">@lang('global.add')</h3>
                    </div>
                    <div class="box-body">
                                ...
                                <div class="tab-content">
                                    @foreach(config('translatable.locales') as $locale)
                                        <div class="tab-pane {{ $locale == app()->getLocale() ? 'active' : '' }}" id="job_tab_{{ $locale }}">
                                            <input type="text" name="job_{{ $locale }}" class="form-control" placeholder="@lang('global.job') - @lang('global.'.$locale)">
                                        </div><!-- /.tab-pane -->
                                    @endforeach
                                </div><!-- /.tab-content -->
                            </div>
